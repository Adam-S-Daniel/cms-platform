...
Rule 3 above is a convention a reviewer has to remember. On cms-platform it is now enforced by the bot: `.github/dependabot.yml` gives **both** ecosystems — `github-actions` and the `/e2e` `npm` harness — a minimum package age, but **not the same shape**, because GitHub does not support one of the keys on one of them:

```yaml
# github-actions — flat only.
cooldown:
  default-days: 7
```

```yaml
# npm (the /e2e harness) — graduated; npm supports the per-SemVer-tier key.
cooldown:
  default-days: 7
  semver-major-days: 30
```

**`semver-major-days` is a schema error on `github-actions`, not a stricter policy — and the error disables the whole `updates[]` entry, not just the key.** GitHub's Dependabot options reference marks per-SemVer-tier cooldown as unsupported for that ecosystem; the error reads `'#/updates/0/cooldown/semver-major-days' is not supported for the package ecosystem 'github-actions'`, and it was hit for real on claude-memory-map's config: https://github.com/Adam-S-Daniel/claude-memory-map/runs/93574227209. That is why cms-platform's own `.github/dependabot.yml` puts `semver-major-days` on the `npm` entry only — the `github-actions` entry stays flat.

The reason both ecosystems are covered at all is blast radius — repairing the Dependabot pipeline (comment-sync + the re-arm sweep) removed the last human gate on a fresh third-party action SHA landing in the 18 reusables **both** production sites execute, so the wait has to be enforced by the bot rather than by whoever happens to review the PR. `github-actions` gets that flat 7-day floor. The `/e2e` harness gets MORE, not the same treatment: it is not platform-internal either — both consumers check it out and execute it, so a bad Playwright major reaches production CI the same way a bad action SHA would — and unlike `github-actions`, npm can express that extra wait as `semver-major-days` instead of just a longer flat number.
...
- **Majors wait 30 days on the `npm` entry, because a major is the class that has actually needed reverting here** — `setup-node` 6→7 (#179) is the incident that surfaced the need, even though `setup-node` itself lives on `github-actions`, which cannot carry a majors-only wait at all (see the schema error above) and so gets only the flat 7 days; the Decap bundle bump is kept revertible on purpose too (v0.1.66 → v0.1.67). Playwright — which `npm` CAN gate this way — is the test RUNNER for the whole `/e2e` suite that both consumers check out and execute in their own production CI, so a Playwright major is the class that most needs deliberate handling rather than a red-CI scramble — 30 days is that room. (An earlier version of this bullet additionally claimed a Playwright major requires a coupled `.github/ci-runner/Dockerfile` bump via the `playwright-image-drift` guard. That coupling does not exist anywhere in the fleet — measured 2026-08-31: no `.github/ci-runner/` or root `package-lock.json` in cms-platform, no `.github/ci-runner/Dockerfile` in either consumer, and `scaffold/create-site.js` seeds neither. Don't reintroduce it on the strength of this bullet's own prior wording.)
