...
- **`CMS_PLATFORM_PAT`** — the legacy fallback those two reusables accept when the App above is absent: `CMS_E2E_PAT`'s permissions **plus Workflows R/W** (the bump rewrites `.github/workflows/*` pins), minus Actions. **A new consumer does not need it**, and both live consumers deleted it (PAT and secret) on 2026-09-02 once the App path was verified. Missing both is issue #13.
